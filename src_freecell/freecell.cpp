...
#ifdef USEOPENGL
void FreecellGame::setupOpenGLArea() {
  #ifdef __linux__
  // Create OpenGL rendering area
  ...
  gtk_widget_set_size_request(
      gl_area_,
      BASE_CARD_WIDTH * 8 + BASE_CARD_SPACING * 9,
...
bool FreecellGame::isOpenGLSupported() const {
  #ifndef USEOPENGL
  return false;
  #else
  return true;
...
bool FreecellGame::setRenderingEngine(RenderingEngine engine) {
  #ifdef USEOPENGL
...
bool FreecellGame::initializeRenderingEngine() {
  #ifndef USEOPENGL
  if (rendering_engine_ == RenderingEngine::OPENGL) {
    rendering_engine_ = RenderingEngine::CAIRO;
  }
...
bool FreecellGame::switchRenderingEngine(RenderingEngine newEngine) {
  #ifndef USEOPENGL
...
std::string FreecellGame::getRenderingEngineName() const {
  switch (rendering_engine_) {
    case RenderingEngine::CAIRO:
      return "Cairo";
    case RenderingEngine::OPENGL:
...
void FreecellGame::printEngineInfo() {
...
void FreecellGame::cleanupRenderingEngine() {
  switch (rendering_engine_) {
    case RenderingEngine::CAIRO:
      cleanupCardCache();
      cairo_initialized_ = false;
...
void FreecellGame::saveEnginePreference() {
  #ifdef __linux__
  std::string preference_file = settings_dir_ + "/engine_preference.txt";
  std::ofstream file(preference_file);
  if (file.is_open()) {
    file << (rendering_engine_ == RenderingEngine::OPENGL ? "opengl" : "cairo");
    file.close();
  }
  #endif
}

void FreecellGame::loadEnginePreference() {
  #ifdef __linux__
  std::string preference_file = settings_dir_ + "/engine_preference.txt";
  ...
  if (file.is_open()) {
    ...
    file >> preference;
    file.close();
    if (preference == "opengl" && isOpenGLSupported()) {
...
#ifdef USEOPENGL
gboolean FreecellGame::onGLRealize(GtkGLArea *area, gpointer data) {
  FreecellGame *game = static_cast<FreecellGame *>(data);
...
gboolean FreecellGame::onGLRender(GtkGLArea *area, GdkGLContext *context, gpointer data) {
  (void)context;
  FreecellGame *game = static_cast<FreecellGame *>(data);
...
#ifdef USEOPENGL
bool FreecellGame::initializeOpenGLResources() {
  #ifdef __linux__
...
void FreecellGame::clearAndRebuildCaches() {
  cache_dirty_ = true;
  cleanupCardCache();
  ...
  if (rendering_engine_ == RenderingEngine::CAIRO) {
    ...
    #ifdef USEOPENGL
    // OpenGL texture initialization happens in renderFrame_gl
    #endif
  }
...
// ============================================================================
// STUB IMPLEMENTATIONS FOR OPENGL METHODS
// ============================================================================
...
void FreecellGame::initializeSettingsDir() {
    ...
    settings_dir_ = std::string(app_data) + "\\Freecell";
    ...
    settings_dir_ = std::string(home) + "/.freecell";
