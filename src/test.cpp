...
int main(int argc, char *argv[]) {
    ...
    if (argc != 3) {
        std::cout << "Usage: " << argv[0] << " <zip_file> <wav_file_in_zip>" << std::endl;
        std::cout << "Example: " << argv[0] << " sounds.zip music/background.wav" << std::endl;
        return 1;
    }
    ...
    std::cout << "Extracting " << wavFileInZip << " from " << zipFile << "..." << std::endl;
    std::vector<uint8_t> wavData;
    if (!extractFileFromZip(zipFile, wavFileInZip, wavData)) {
        std::cerr << "Failed to extract WAV file from ZIP archive. Exiting." << std::endl;
        return 1;
    }
    ...
    if (!AudioManager::getInstance().loadSoundFromMemory(SoundEvent::CardPlace, wavData, format)) {
