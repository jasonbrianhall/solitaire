...
std::vector<CardImage> load_cards_from_zip(const char *zipPath) {
  ...
  if (!archive) {
    ...
    std::cerr << "Error opening " << zipPath << ": "
  ...
  std::sort(
      cards.begin(), cards.end(), [](const CardImage &a, const CardImage &b) {
...
static GtkWidget *create_card_widget(const CardImage &card) {
  ...
  write_success = gdk_pixbuf_loader_write(loader, card.data.data(),
                                          card.data.size(), &error);
  ...
  GdkPixbuf *scaled =
      gdk_pixbuf_scale_simple(pixbuf, 150, 218, GDK_INTERP_BILINEAR);
  g_object_unref(loader);
  ...
  std::string label =
      card.card_info ? card.card_info->toString() : card.filename;
...
static void activate(GtkApplication *app, gpointer user_data) {
  ...
  std::vector<CardImage> cards =
      load_cards_from_zip(app_data->zip_path.c_str());
...
int main(int argc, char **argv) {
  ...
  if (argc > 1) {
    ...
    if (zip_path.empty()) {
      std::cerr
          << "Error: Could not find cards.zip in current or parent directory.\n"
          << "Please provide the path to the ZIP file as an argument."
          << std::endl;
      return 1;
    }
  }
